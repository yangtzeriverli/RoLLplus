...
step=1
#比较的对象是地址空间中IP地址的个数
f3=open("addressspaceparameter2022.txt",'w')
...
# 1024到pow(2,16)
# 最大和是: 1.37533330320423
# 对应的阈值是: 256




#这里遵循一个假设是:地址空间大的位于较靠上的位置，地址空间较小的在底层

maxsum=0
maxthrehold=0

while flatthrehold<=pow(2,16):
    legtripledict = defaultdict(int)
    f=open("legitimateaddressspace2022.txt")
    line=f.readline()
    while line:
        if 'unknown' not in line and '-1' not in line:
            tmp=line.split("|")
            a1=eval(tmp[0])
            a2=eval(tmp[1])
            a3=eval(tmp[2])
    ...
    f=open("illegitimateaddressspace2022.txt")
    line=f.readline()
    while line:
        if 'unknown' not in line and '-1' not in line:
            tmp=line.split("|")
...
# 7245|0.7487684729064039|0.6100917431192661 选这组看起来好一点